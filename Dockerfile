...
# and `pip install rag_skeleton` using an artifact store. Faster and more robust.
...
SHELL ["mamba", "run", "-n", "rag_skeleton", "/bin/bash", "-c"]

# Build package
RUN tox -e build
RUN mamba env export -n rag_skeleton -f environment.docker.yml
...
SHELL ["conda", "run", "-n", "rag_skeleton", "/bin/bash", "-c"]
...
# ENTRYPOINT ["conda", "run", "--no-capture-output", "-n", "rag_skeleton", "YOUR_CONSOLE_SCRIPT"]
